...
def handle_register(args):
    ...
    print(f"Пользователь '{result['username']}' зарегистрирован (id={result['user_id']}). Войдите: login --username {result['username']} --password ****")
...
def handle_buy(args):
    ...
    print(f"Покупка выполнена: {format_balance(result['amount'], currency)} {currency} по курсу {format_money(result['rate'])} USD/{currency}")
    print('Изменения в портфеле:')
    print(f"- {currency}: было {format_balance(result['wallet_before'], currency)} → стало {format_balance(result['wallet_after'], currency)}")
    print(f"- USD: было {format_money(result['usd_before'])} → стало {format_money(result['usd_after'])}")
...
def handle_sell(args):
    ...
    print(f"Продажа выполнена: {format_balance(result['amount'], currency)} {currency} по курсу {format_money(result['rate'])} USD/{currency}")
    print('Изменения в портфеле:')
    print(f"- {currency}: было {format_balance(result['wallet_before'], currency)} → стало {format_balance(result['wallet_after'], currency)}")
    print(f"- USD: было {format_money(result['usd_before'])} → стало {format_money(result['usd_after'])}")
...
def handle_get_rate(args):
    ...
    print(f"Курс {result['from']}→{result['to']}: {result['rate']:.8f} (обновлено: {result['updated_at']})")
